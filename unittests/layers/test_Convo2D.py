...
from dnnviewerlib.layers.Convo2D import Convo2D
...
class TestConvo2D:
    ...
    def test_plot_topn_connections_backward(self):
        ...
        # Max on conv filters
        # [[9, 2, 6], [5, 6, 8]]
        layer = Convo2D('test_1', 3, weights)
        layer.set_xoffset(10)

        prev_layer = Dense('test_prev', 2, np.ones((3, 2)))  # weights of other do no matter
        ...
        assert len(shapes) == 4 # Each Convo to the top Dense
    ...
    def test_plot_topn_connections_forward(self):
        ...
        layer = Convo2D('test_2', 3, weights)
        layer.set_xoffset(10)
        prev_layer = Dense('test_prev', 2, np.ones((3, 2)))  # weights of other do no matter
        prev_layer.set_xoffset(0)
        ...
        assert len(shapes) == 2 # For each active Dense to each Convo top 2
        assert isinstance(shapes[0], dict)
        assert shapes[0]['type'] == 'path'
