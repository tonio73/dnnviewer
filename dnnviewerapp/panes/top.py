    ...
    def callbacks(self):
        ...
        @app.callback(Output('top-epoch-index', 'data'),
                     [Input('top-step-backward', 'n_clicks'),
                      ...
                      # There is normaly no event trigger on load BUT it seems that there is one, the latest
                      Input('top-fast-backward', 'n_clicks')])
        def fast_backward(clicks_fb, clicks_sb, clicks_ff, clicks_sf):
            ...
            print('Triggering', button_id)

            if button_id == 'top-fast-backward' or button_id == 'no_clicks':
