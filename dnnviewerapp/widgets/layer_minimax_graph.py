...
def figure(weights, num_unit, unit_names, plotly_theme, title):
    ...
    fig = go.Figure(data=[go.Bar(x=w_min, hovertext=hover_text_min, hoverinfo='text',
                          ...
                          go.Line(x=w_mean, hovertext=hover_text_max, hoverinfo='text',
                                 customdata=['layer'], name='Mean')
                          ])
    fig.update_layout(margin=dict(l=10, r=10, b=30, t=40),  # noqa: E741
                      title_text='Unit min-max ' + str(title),
